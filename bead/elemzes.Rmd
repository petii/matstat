...
Az adatokban a pokémonok, melyeknek különböző formái vannak, külön pokémonoknak számítanak. A duplikátumok elkerülése végett, ezeket a formákat kiszűrtem, a tulajdonságaikat pedig átlagoltam.

```{r}
data.Name <- gsub("(.)([[:upper:]])", "\\1 \\2", data$Name)
for (i in data.Name) {
  if (!grepl('.',i,fixed = TRUE)) { # Mr. Mime and Mime Jr.
    r <- strsplit(i,' ')[[1]]
    if (length(r) > 1) {
      #print(r[1])
      s <- data[grepl(r[1],data$Name,fixed = TRUE),]
      levels(data$Name) <- c(levels(data$Name),r[1])
      ...
      base$Name <- r[1]
      for (l in statlab) {
        tmp.avg.stat[[l]] <- mean(s[[l]])
        base[[l]] <- tmp.avg.stat[[l]]
      }
      data<-data[!grepl(r[1],data$Name,fixed = TRUE),]
...
gen5 <- normal.gens$'5'
gen6 <- normal.gens$'6'
boxplot(gen1$Total,gen2$Total,gen3$Total,gen4$Total,gen5$Total,gen6$Total,main='Generációk össz tulajdonságai',names = names(data.gens))
layout(matrix(c(1:6),2,3,byrow = TRUE))
for (i in stats.names) {
  boxplot(gen1[[i]],gen2[[i]],gen3[[i]],gen4[[i]],gen5[[i]],gen6[[i]],main=i,names = names(data.gens))
}
```
